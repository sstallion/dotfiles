...
return {
  ...
  init = function()
    -- Copied from lua/nvim-tree/view.lua:
    ...
    -- exception.
    local function create_alt(fn, ...)
      if is_floating() and vim.bo.filetype ~= 'NvimTree' then
    ...
    vim.api.nvim_create_user_command('NvimTreeOpenAlt', function()
      local api = require('nvim-tree.api')
      local path = vim.fn.expand('%:p:h')
      create_alt(api.tree.open, { path = path, find_file = true })
    end, { desc = 'Opens the tree (alternate)' })

    vim.api.nvim_create_user_command('NvimTreeToggleAlt', function()
      local api = require('nvim-tree.api')
      create_alt(api.tree.toggle, { path = vim.fn.getcwd(), find_file = true })
    end, { desc = 'Open or close the tree (alternate)' })
  end,
  config = function()
    local api = require('nvim-tree.api')
    local view = require('nvim-tree.view')

    -- An alternate hijacking method is needed to support editing directories
    -- in a floating window. The directory buffer is removed after entry and
    -- the tree is scheduled to be focused after the autocommand completes.
      ...
      view = {
        float = {
          ...
          open_win_config = function()
            ...
            local width = math.min(scalewidth(winid, 1.0), 38) - 4 -- fit between separators
  ...
  keys = {
    { '<Leader>t', '<Cmd>NvimTreeToggleAlt<CR>' },
    { '-',         '<Cmd>NvimTreeOpenAlt<CR>' },
  },
}
